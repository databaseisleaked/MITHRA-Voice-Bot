...
#changes to handle all kinda prompts are done here
...
def ask():
    try:
        ...
        # ✅ Gemini: Classify user query type
        def classify_query_type(user_input):
            try:
                prompt = f"""
You are a query classification assistant for the Chennai Metro chatbot.
Your job is to detect what kind of user query this is.
Classify the following user input into ONE of these categories:

1. ROUTE_QUERY: If the user is asking about travel between two stations (A to B).
2. STATION_INFO: If the user is asking for details about platforms, lifts, escalators, or exits at one or more stations.
3. OTHER: Anything else.

Respond with ONLY one of these labels: ROUTE_QUERY, STATION_INFO, or OTHER.

User input: "{user_input}"
"""
                response = model.generate_content(prompt)
                result = response.text.strip().upper()
                if result in ["ROUTE_QUERY", "STATION_INFO", "OTHER"]:
                    return result
                return "OTHER"
        ...
        # ✅ Gemini: Extract only relevant part of station info
        def extract_specific_info_with_gemini(user_query, station_name, station_details):
            try:
                prompt = f"""
You are an assistant that filters metro station information.

Given a user query and the full raw details of a station, extract and return ONLY the part of the station data that directly answers the user's query.

DO NOT rephrase or summarize — return the specific lines or sentences that match the query exactly.

If the user asks for escalators, return only escalator-related lines. If lifts, return only lift-related lines. If platform number, return only that. If exits, return only that.

Be precise and return in raw format.
...
"""
                ...
                print(f"❌ Error in extract_specific_info_with_gemini: {e}")
                return station_details  # Fallback
        ...
        print("🚉 Extracted Stations:", extraction_response)
        print("1111111111111111111111111111111111111111111111111111111111111111111111111111")

        # ✅ Handle STATION_INFO queries first
        if query_type == "STATION_INFO":
            if single_station:
                details = get_station_details(single_station)
                if details:
                    filtered = extract_specific_info_with_gemini(user_input, single_station, details)
                    return jsonify({'response': f"{single_station}: {filtered}"})
                else:
                    return jsonify({'error': f"Could not find details for {single_station}"}), 400

            if source and destination:
                src_details = get_station_details(source)
                dest_details = get_station_details(destination)

                if not src_details and not dest_details:
                    return jsonify({'error': f"No platform info found for {source} or {destination}"}), 400

                response_parts = []
                if src_details:
                    filtered_src = extract_specific_info_with_gemini(user_input, source, src_details)
                    response_parts.append(f"{source}:\n{filtered_src}")
                if dest_details:
                    filtered_dest = extract_specific_info_with_gemini(user_input, destination, dest_details)
                    response_parts.append(f"{destination}:\n{filtered_dest}")

                return jsonify({'response': "\n\n".join(response_parts)})

            # ✅ Fallback: Try inferring station name if nothing extracted
            inferred_station_prompt = f"""
You are an intelligent assistant. Given this user query, figure out if the user is referring to a specific Chennai Metro station.

Extract and return ONLY the station name (no explanation, just the name). If nothing is identifiable, return "UNKNOWN".

User Query: "{user_input}"
"""
            response = model.generate_content(inferred_station_prompt)
            inferred_station = response.text.strip()
            print("🔍 Inferred Station Name:", inferred_station)

            if inferred_station and inferred_station.upper() != "UNKNOWN":
                inferred_details = get_station_details(inferred_station)

                # Fallback DB check (case-insensitive)
                # Fallback DB check (case-insensitive)
                if not inferred_details:
                    try:
                        conn = sqlite3.connect("route_data.db")
                        cursor = conn.cursor()
                        # Try partial match in station_data
                        cursor.execute("SELECT name, info FROM station_data WHERE LOWER(name) LIKE ?", ('%' + inferred_station.lower() + '%',))
                        row = cursor.fetchone()
                        conn.close()
                        if row:
                            inferred_station = row[0]
                            inferred_details = row[1]
                        else:
                            print("❌ No matching station info found for:", inferred_station)
                    except Exception as db_err:
                        print("❌ DB Fallback Error:", db_err)


                if inferred_details:
                    filtered = extract_specific_info_with_gemini(user_input, inferred_station, inferred_details)
                    return jsonify({'response': f"{inferred_station}: {filtered}"})
                else:
    # Try with blue+green line data as generic fallback
                    blue = get_blue_line_stations()
                    green = get_green_line_stations()
                    fallback_details = "\n".join([f"{name}: {dist} km" for name, dist in blue + green])
                    
                    filtered = extract_specific_info_with_gemini(user_input, inferred_station, fallback_details)
                    return jsonify({'response': f"(Partial Match - {inferred_station}): {filtered}"})
            else:
                return jsonify({'error': 'Could not extract station information. Please provide a valid station name.'}), 400

        # ✅ Proceed if it's a valid route query
        route_data = None
        if query_type == "ROUTE_QUERY" and source and destination:
            ...
            route_data = {
                ...
                "Interchange": row[4],
                "Source Platform Details": source_station_details,
                "Destination Platform Details": destination_station_details,
                "Line Color": row[7],
                "Line Symbol": row[8],
            ...
            print("Debugging the route data's flowwwwwwwwwwwwwwwwwwwwwwwwwwww", route_data)
...
The stations are : {blue_line_data}, {green_line_data}
