document.addEventListener("DOMContentLoaded", () => {
  ...
  const stationAliases = {
   ...
   "MGR central": "Puratchi Thalaivar Dr. M.G. Ramachandran Central (Chennai Central)",  // ✅ Fix for your issue
    ...
    "Arignar Anna Alandur": "Alandur",
      "Arignar Anna Alandur (Alandur)": "Alandur",
      "Puratchi Thalaivar Dr. M.G. Ramachandran Central": "Chennai Central",
  ...
  //changed ekkatuthanganl to ekkattuthangal
    let greenLine = [
        "St. Thomas Mount", "Arignar Anna Alandur (Alandur)", "Ekkatuthangal", "Ashok Nagar",
    ...
    if (greenLine.includes(src) && blueLine.includes(dest)) {
      firstInterchange = "Arignar Anna Alandur (Alandur)";
      console.log("✅ First possible interchange:", firstInterchange);
        routeViaAlandur = getDirectPath(greenLine, src, firstInterchange)
            .concat(getDirectPath(blueLine, firstInterchange, dest).slice(1));
  
        
            secondInterchange = "Puratchi Thalaivar Dr. M.G. Ramachandran Central (Chennai Central)";
            console.log("✅ Alternate interchange:", secondInterchange);
            ...
            let finalRoute = routeViaCentral.length > 0 ? routeViaCentral : routeViaAlandur;
            console.log("The final interchange point is", finalRoute.find(station => station === secondInterchange) || firstInterchange);
    ...
    if (blueLine.includes(src) && greenLine.includes(dest)) {
      ...
      console.log("✅ First possible interchange:", firstInterchange);
        routeViaAlandur = getDirectPath(blueLine, src, firstInterchange)
            .concat(getDirectPath(greenLine, "Arignar Anna Alandur (Alandur)", dest).slice(1));
            secondInterchange = "Puratchi Thalaivar Dr. M.G. Ramachandran Central (Chennai Central)"; // Alternate interchange
            console.log("✅ Alternate interchange:", secondInterchange);
        routeViaCentral = getDirectPath(blueLine, src, secondInterchange)
            ...
            let finalInterchange = finalRoute.find(station => station === secondInterchange) || firstInterchange;
  console.log("🚉 The final interchange point is", finalInterchange);
  
  // Prepare the data to send
  let requestData = {
      source: src,  // Source station
      destination: dest,  // Destination station
      path: finalRoute,  // Complete path array
      interchange: finalInterchange  // The final interchange station
  };
  ...
  fetch('/receive_route_data', {
      ...
      body: JSON.stringify(requestData)  // Send all data as JSON
  })
  .then(response => response.json())
  .then(data => {
